...
(Joomla => {
  ...
  class Searchtools {
    constructor(elem, options) {
      ...
      this.orderCols.forEach(item => {
        item.addEventListener('click', event => {
          const element = event.target.tagName.toLowerCase() === 'span' ? event.target.parentNode : event.target; // Order to set

          const newOrderCol = element.getAttribute('data-order');
          const newDirection = element.getAttribute('data-direction');
          const newOrdering = "".concat(newOrderCol, " ").concat(newDirection); // The data-order attribute is required
    ...
    activeFilter(element) {
      ...
      const chosenId = "#".concat(element.getAttribute('id'));
