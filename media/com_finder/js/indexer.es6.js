...
((Joomla, document) => {
  ...
  Joomla.finderIndexer = () => {
    ...
    const token = "&".concat(document.getElementById('finder-indexer-token').getAttribute('name'), "=1");
        ...
        if (json.start) {
          ...
          if (document.getElementById('finder-debug-data')) {
            ...
            Object.entries(json.pluginState).forEach(context => {
              let item = "<dt class=\"col-sm-3\">".concat(context[0], "</dt>");
    ...
    const handleFailure = xhr => {
      ...
      const message = data ? data.message : "".concat(Joomla.JText._('COM_FINDER_MESSAGE_RETURNED'), "<br>").concat(data);
