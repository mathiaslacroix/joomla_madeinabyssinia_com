...
(function (Joomla, doc) {
  ...
  function fade(fadeAction, transitAction) {
    var sidebar = doc.querySelector('.sidebar-wrapper');
    var sidebarChildren = sidebar ? sidebar.children : [];
    var sideChildrenLength = sidebarChildren.length;
    var contentMain = doc.querySelector('.container-main');
    var contentChildren = contentMain ? contentMain.children : [];
    var contChildrenLength = contentChildren.length;

    for (var i = 0; i < sideChildrenLength; i += 1) {
      sidebarChildren[i].classList.add("load-fade".concat(fadeAction));
    }

    for (var _i = 0; _i < contChildrenLength; _i += 1) {
      contentChildren[_i].classList.add("load-fade".concat(fadeAction));
    }

    if (sidebar) {
      if (transitAction) {
        // Transition class depends on the width of the sidebar
        if (storageEnabled && localStorage.getItem('atum-sidebar') === 'closed') {
          sidebar.classList.add("transit-".concat(transitAction, "-closed"));
          changeLogo('small');
        } else {
          sidebar.classList.add("transit-".concat(transitAction));
        }
      }
  ...
  function fadeLoginLogout() {
    ...
    if (loginForm) {
      loginForm.addEventListener('joomla:login', function () {
        fade('out', 'narrow');
      });
    } else {
      // Fade out dashboard on logout
      var logoutBtn = doc.querySelector('.header-items a[href*="task=logout"]');
