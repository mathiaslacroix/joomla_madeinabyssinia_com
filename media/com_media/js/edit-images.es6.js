...
(() => {
  ...
  Joomla.submitbutton = task => {
    ...
    const url = "".concat(options.apiBaseUrl, "&task=api.files&path=").concat(uploadPath);
      ...
      links.forEach(link => {
        link.addEventListener('joomla.tab.shown', event => {
          ...
          if (event.relatedTarget) {
            Joomla.MediaManager.Edit[event.relatedTarget.id.replace('tab-attrib-', '').toLowerCase()].Deactivate(); // Clear the DOM
