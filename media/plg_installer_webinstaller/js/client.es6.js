...
((window, document, Joomla, jQuery) => {
  ...
  class WebInstaller {
    ...
    loadweb(url) {
      ...
      let requestUrl = "".concat(url, "&product=").concat(webInstallerOptions.options.product, "&release=").concat(webInstallerOptions.options.release, "&dev_level=").concat(webInstallerOptions.options.dev_level, "&list=").concat(webInstallerOptions.list ? 'list' : 'grid', "&lang=").concat(webInstallerOptions.options.language);
    ...
    installfromwebajaxsubmit() {
      let tail = "&view=".concat(webInstallerOptions.view);
