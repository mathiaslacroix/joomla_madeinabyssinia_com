...
((window, document, Joomla) => {
  ...
  Joomla.sampledataAjax = (type, steps, step) => {
    // Get variables
    const baseUrl = "index.php?option=com_ajax&format=json&group=sampledata&".concat(Joomla.getOptions('csrf.token'), "=1");
    ...
    Joomla.request({
      ...
      onSuccess: resp => {
        ...
        if (response.success && response.data && response.data.length > 0) {
          const progress = document.querySelector(".sampledata-progress-".concat(type, " .progress-bar")); // Display all messages that we got
  ...
  Joomla.sampledataApply = element => {
    ...
    const progressElements = [].slice.call(document.querySelectorAll(".sampledata-progress-".concat(type)));
