...
(document => {
  ...
  document.addEventListener('joomla:updated', event => {
    ...
    const target = event.target; // Check is it subform, then wee need to fix some "showon" config
    ...
    if (target.classList.contains('subform-repeatable-group')) {
      ...
      const search = new RegExp("\\[".concat(baseName, "\\]\\[").concat(baseName, "X\\]"), 'g');
      const replace = "[".concat(baseName, "][").concat(group, "]"); // Fix showon field names in a current group
