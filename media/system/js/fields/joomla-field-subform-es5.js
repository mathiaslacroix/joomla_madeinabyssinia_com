...
(function (customElements) {
  ...
  function (_HTMLElement) {
    ...
    function JoomlaFieldSubform() {
      ...
      _this = _possibleConstructorReturn(this, _getPrototypeOf(JoomlaFieldSubform).call(this));
      ...
      if (_this.rowsContainer) {
        ...
        for (var i = 0, l = allContainers.length; i < l; i++) {
          if (closest(allContainers[i], 'joomla-field-subform') === _assertThisInitialized(_this)) {
      ...
      if (_this.buttonAdd || _this.buttonRemove) {
        ...
        _this.addEventListener('keydown', function (event) {
          ...
          var isAdd = that.buttonAdd && event.target[matchesFn](that.buttonAdd);
          var isRem = that.buttonRemove && event.target[matchesFn](that.buttonRemove);

          if ((isAdd || isRem) && closest(event.target, 'joomla-field-subform') === that) {
            var row = closest(event.target, that.repeatableElement);
            row = closest(row, 'joomla-field-subform') === that ? row : null;
    ...
    _createClass(JoomlaFieldSubform, [{
      ...
      value: function getRows() {
        ...
        var matchesFn = document.body.msMatchesSelector ? 'msMatchesSelector' : 'matches';
      ...
      value: function fixUniqueAttributes(row, count) {
        ...
        var groupnew = basename + count; // new group name
        ...
        haveName = [].slice.call(haveName).filter(function (el) {
          return closest(el, 'joomla-field-subform') === _this2;
        });

        for (var i = 0, l = haveName.length; i < l; i++) {
          var $el = haveName[i];
          var name = $el.getAttribute('name');
          var id = name.replace(/(\[\]$)/g, '').replace(/(\]\[)/g, '__').replace(/\[/g, '_').replace(/\]/g, '').replace(/\W/g, '_'); // id from name

          var nameNew = name.replace("[".concat(group, "]["), "[".concat(groupnew, "][")); // New name

          var idNew = id.replace(group, groupnew); // Count new id
          ...
          if ($el.type === 'checkbox' && name.match(/\[\]$/)) {
            ...
            if (!countMulti) {
              ...
              var fieldset = closest($el, 'fieldset.checkboxes');
            ...
            if (!countMulti) {
              // Set the id for fieldset and group label
              var _fieldset = closest($el, 'fieldset.radio');
      ...
      value: function setUpDragSort() {
        ...
        this.addEventListener('touchstart', function (event) {
          ...
          var row = handler ? closest(handler, that.repeatableElement) : null;
        ...
        this.addEventListener('keydown', function (event) {
          if (event.keyCode !== KEYCODE.ESC && event.keyCode !== KEYCODE.SPACE && event.keyCode !== KEYCODE.ENTER || event.target.form || !event.target[matchesFn](that.repeatableElement)) {
            return;
          }
        ...
        this.addEventListener('dragstart', function (event) {
          if (item) {
            ...
            event.dataTransfer.effectAllowed = 'move'; // This need to work in Firefox and IE10+
        ...
        this.addEventListener('dragenter', function (event) {
          ...
          var row = event.target[matchesFn](that.repeatableElement) ? event.target : closest(event.target, that.repeatableElement);
          if (!row) return;
          switchRowPositions(item, row);
